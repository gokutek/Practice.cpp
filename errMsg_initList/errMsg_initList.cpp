...
struct ErrCode 
{
	...
	std::string msg()                 // member function of ErrCode
	{
		std::ostringstream s; 
		s << "ErrCode " << num;
		return s.str();
...
std::vector<std::string> functionX()
{
	std::string expected = "description", actual = "some other case";
	// . . .
	if (expected.empty())
...
{
	std::string expected = "description", actual = "some other case";
	std::initializer_list<int> li = { 0,1,2,3 };
